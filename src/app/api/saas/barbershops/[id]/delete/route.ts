...
export async function POST(
  ...
  // ⚠️ IMPORTANTE: se existirem FKs (appointments, barbers, services etc),
  // esse delete pode falhar. O ideal é ter ON DELETE CASCADE nas tabelas filhas.
  const { error } = await admin.from("barbershops").delete().eq("id", params.id);
