...
export default function MinhaBarbeariaPage() {
  ...
  // ✅ base do app (prod/local) — usado p/ mostrar URL completa pro cliente
  const appBaseUrl = useMemo(() => {
    // preferir env no client
    const envUrl = process.env.NEXT_PUBLIC_APP_URL?.trim();
    if (envUrl) return envUrl.replace(/\/$/, "");

    // fallback: pega do navegador
  ...
  const landingFullUrl = useMemo(() => {
    if (!landingPath || !appBaseUrl) return null;
    return `${appBaseUrl}${landingPath}`;
  }, [appBaseUrl, landingPath]);

  const bookingFullUrl = useMemo(() => {
    if (!bookingPath || !appBaseUrl) return null;
    return `${appBaseUrl}${bookingPath}`;
  }, [appBaseUrl, bookingPath]);
        ...
        <header className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          <div>
            ...
            <p className="text-zinc-400 mt-2">
              {shop?.name ? (
                <>
                  Gerencie as informações da{" "}
                  <span className="text-zinc-200 font-semibold">{shop.name}</span>.
                </>
              ) : (
            ...
            {/* ✅ bloco profissional: URL para divulgar */}
            {landingFullUrl && (
              <div className="mt-3 bg-zinc-950 border border-white/10 rounded-xl p-4">
                <p className="text-sm text-zinc-300 font-semibold">
                  Esta é a URL para você divulgar para seus clientes:
                </p>
                ...
                <div className="mt-2 flex flex-col sm:flex-row gap-2 sm:items-center">
                  <a
                    href={landingFullUrl}
                    target="_blank"
                    className="text-yellow-300 font-black break-all hover:underline"
                    rel="noreferrer"
                  >
                    {landingFullUrl}
                  </a>
                  ...
                  <button
                    type="button"
                    onClick={() => copy(landingFullUrl)}
                    className="sm:ml-auto px-4 py-2 rounded-lg bg-white/10 hover:bg-white/15 transition font-black"
                  >
                    Copiar link
                  </button>
                </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-500 mt-1">
                  Usado na landing e no agendamento. Após salvar, os links mudam.
                </p>
              </div>
