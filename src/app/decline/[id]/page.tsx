...
function isAllowedStatus(status: string | null) {
  ...
  return ALLOWED_TO_DECLINE.includes(st as (typeof ALLOWED_TO_DECLINE)[number]);
}

export default function DeclineAppointmentPage() {
  ...
  async function handleDecline() {
    ...
    const { error } = await supabase
      ...
      .eq("id", id)
      .eq("action_token", token)
      .in("status", [...ALLOWED_TO_DECLINE]);

    if (error) {
      setMode("error");
      setMsg("Erro ao recusar: " + error.message);
      setSubmitting(false);
      return;
    }

    setMode("success");
    setMsg("✅ Ok. Você recusou esse horário.");
    setSubmitting(false);
  }
      ...
      <div className="w-full max-w-xl rounded-[28px] border border-white/10 bg-black/40 backdrop-blur-md shadow-[0_25px_90px_rgba(0,0,0,0.65)] overflow-hidden">
        <div className="p-6 sm:p-8 border-b border-white/10">
          ...
          <p className="mt-2 text-white/65 text-sm">
            Use isso se o horário sugerido não serve pra você.
          </p>
        </div>
              ...
              <div className="mt-6 flex flex-col gap-3">
                {mode === "idle" && isAllowedStatus(appt?.status ?? null) && (
                  <button
                    ...
                    disabled={submitting}
                    className={clsx(
                      "h-12 w-full rounded-2xl font-black text-white transition",
                      "bg-red-500/85 hover:bg-red-500 disabled:opacity-50",
                      "shadow-[0_0_0_1px_rgba(255,80,80,0.35),0_18px_55px_rgba(0,0,0,0.65)]",
                    )}
                  >
