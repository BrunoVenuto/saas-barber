...
export default function DeclinePage() {
  ...
  useEffect(() => {
    async function run() {
      ...
      const { data, error } = await supabase.rpc("rpc_cancel_appointment", {
        p_appointment_id: id,
        p_token: token,
      });
      ...
      if (error) {
        ...
        setOk(false);
        setMsg(
          "Não consegui cancelar. O link pode ter expirado ou já foi usado.",
        );
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-black text-white px-4 py-10">
      <div className="mx-auto max-w-md rounded-3xl border border-white/10 bg-white/5 p-5 shadow-[0_20px_70px_rgba(0,0,0,0.65)]">
        <p className="text-xs text-white/60">Cancelamento</p>
        <h1 className="mt-1 text-2xl font-black">Cancelar agendamento</h1>

        <div className="mt-5 rounded-2xl border border-white/10 bg-black/30 p-4">
          {loading ? (
            <p className="text-white/70">Cancelando...</p>
          ) : (
            <>
              <p
                className={clsx(
