...
export default function AcceptRescheduleRequestPage() {
  ...
  useEffect(() => {
    async function run() {
      ...
      if (error) {
        ...
        setOk(false);
        setMsg(
          "Não consegui aceitar. O link pode ter expirado ou já foi usado.",
        );
        return;
      }
      ...
      setMsg(
        Boolean(data)
          ? "✅ Remarcação aceita! Seu agendamento foi atualizado."
  ...
  return (
    <div className="min-h-screen bg-black text-white px-4 py-10">
      <div className="mx-auto max-w-md rounded-3xl border border-white/10 bg-white/5 p-5 shadow-[0_20px_70px_rgba(0,0,0,0.65)]">
        ...
        <h1 className="mt-1 text-2xl font-black">Aceitar sugestão</h1>

        <div className="mt-5 rounded-2xl border border-white/10 bg-black/30 p-4">
          {loading ? (
            <p className="text-white/70">Aplicando remarcação...</p>
          ) : (
            <>
              <p
                className={clsx(
